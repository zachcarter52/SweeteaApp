...
function createAnyOfWithDiscriminator(schemas, discriminatorMap, discriminatorField) {
  return {
    type: function () {
      return "OneOf<".concat(schemas.map(function (schema) {
        return schema.type();
      }).join(' | '), ">");
    },
...
function matchAndMap(schemas, value, ctxt) {
  ...
  return matchedSchemas.length > 0 ? matchedSchemas[0].map(value, ctxt) : undefined;
}
function matchAndUnmap(schemas, value, ctxt) {
  ...
  return matchedSchemas.length > 0 ? matchedSchemas[0].unmap(value, ctxt) : undefined;
}
function matchAndMapXml(schemas, value, ctxt) {
  ...
  return matchedSchemas.length > 0 ? matchedSchemas[0].mapXml(value, ctxt) : undefined;
}
function matchAndUnmapXml(schemas, value, ctxt) {
  ...
  return matchedSchemas.length > 0 ? matchedSchemas[0].unmapXml(value, ctxt) : undefined;
}
export { anyOf };